...
from demofold.utils.exponential_moving_average import ExponentialMovingAverage
...
from demofold.utils.logger import PerformanceLoggingCallback
from demofold.utils.import_weights import import_openfold_weights_


class DemoFoldWrapper(pl.LightningModule):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.model = DemoFold(config)
        
        if config.globals.is_e2e:
            self.loss = StructureLoss(config.struct_loss)
        else:
...
def main(args):
    ...
    if(args.deepspeed_config_path is not None):
        ...
        if(args.wandb):
            ...
            wdb_logger.experiment.save("demofold/config.py")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Directory containing precomputed training ss"
    )
    parser.add_argument(
        "output_dir", type=str,
    ...
    parser.add_argument(
        "--train_epoch_len", type=int, default=5000, # todo openfold是10000
        help=(
